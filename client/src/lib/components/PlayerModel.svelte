<script lang="ts">
	...
	const positionArray = $derived(Array.isArray(position) ? position : [position.x, position.y, position.z] as [number, number, number]);

	let meshRef = $state<any>();
	...
	function handlePointerMove(event: any) {
		...
		const newPosition = {
			x: (position as any).x + deltaX,
			y: (position as any).y || 1,
			z: (position as any).z + deltaZ
		};
